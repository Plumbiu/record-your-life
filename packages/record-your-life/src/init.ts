...
function insertRecord(name: string | undefined, shouldUpdate = false) {
  if (!name) {
    return
  }
...
export async function init(timer: number, config: Config) {
  ...
  let unkownPreApp: string | undefined
  WatchWindowForeground(async (_curr, prevId, win) => {
    ...
    insertRecord(preApp ?? unkownPreApp, true)
    insertRecord(curApp ?? win.title)
    unkownPreApp = win.title
    if (curApp) {
      shoudWrite = true
    }
  })
